...
- 💾 **可配置数据目录（配置持久化）**：新增 `mdataDir` 环境变量，把系统配置、咪咕账号、外部订阅源、我的频道（分组顺序/隐藏/归类/排序）等运行时数据集中存放；Docker 挂载卷（`-v ./data:/migu/data`）后，**重建容器 / 升级镜像不再丢失配置**（默认仍为当前目录，向后兼容；旧部署升级前建议用 `docker cp` 备份配置，详见部署说明）
