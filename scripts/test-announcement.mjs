...
import {
  ANNOUNCEMENT, ANNOUNCEMENT_CHANNEL_KEY, SYSTEM_CHANNELS, SYSTEM_CHANNEL_KEYS, YSP_LOGIN_GUIDE,
...
assert.deepEqual(SYSTEM_CHANNELS.map(channel => channel.tvgId), ['iptv-announcement', 'iptv-ysp-login-guide'])
assert.deepEqual(SYSTEM_CHANNEL_KEYS, ['公告::iptv-announcement', '公告::iptv-ysp-login-guide'])
assert.equal(ANNOUNCEMENT_CHANNEL_KEY, systemChannelKey(ANNOUNCEMENT))

// 两条系统频道都写进播放列表，公告在前、教程在后，同一分组
...
assert.equal((m3u.match(/#EXTINF/g) || []).length, 2)
...
// 两条短片都必须是 1080p、faststart 的 H.264 mp4，台标 512×512 PNG
...
const protectedConfig = protectAnnouncementConfig({
  ...
  groupOrder: ['体育', '公告', '央视'],
  channelGroupMap: { [ANNOUNCEMENT_CHANNEL_KEY]: '影视', [GUIDE_KEY]: '影视', '央视::c1': '收藏' },
  channelRenameMap: { [ANNOUNCEMENT_CHANNEL_KEY]: '别删我', [GUIDE_KEY]: '改名' },
  channelOrder: { 公告: [GUIDE_KEY, ANNOUNCEMENT_CHANNEL_KEY], 央视: ['央视::c1'] },
...
const hostileConfig = () => ({
  ...
  deletedGroups: ['公*'],
  groupOrder: ['央视', '公告'],
  channelGroupMap: { [ANNOUNCEMENT_CHANNEL_KEY]: '央视', [GUIDE_KEY]: '央视' },
  channelRenameMap: { [ANNOUNCEMENT_CHANNEL_KEY]: '伪公告', [GUIDE_KEY]: '伪教程' },
  channelOrder: { 公告: [GUIDE_KEY, ANNOUNCEMENT_CHANNEL_KEY] }, groupRenameMap: { 公告: '其它' }, groupSortMode: {}, customGroups: [],
})

// 删除 / 移动 / 重命名 / 改序全部无效：公告分组仍在首位，两条短片按声明顺序、原名输出
const forced = applyConfig(sampleGroups(), hostileConfig())
assert.equal(forced[0].name, ANNOUNCEMENT.group)
assert.deepEqual(forced[0].channels.map(channel => channel.name), [ANNOUNCEMENT.name, YSP_LOGIN_GUIDE.name])

// 隐藏是唯一放行的操作（issue #119），且逐条独立：藏掉教程，公告照常置顶
const guideHidden = applyConfig(sampleGroups(), { ...hostileConfig(), hiddenChannels: [GUIDE_KEY] })
assert.deepEqual(guideHidden[0].channels.map(channel => channel.id), [ANNOUNCEMENT.tvgId])
// 两条都藏掉，公告分组整个不出现，其它分组不受影响
const hidden = applyConfig(sampleGroups(), { ...hostileConfig(), hiddenChannels: [ANNOUNCEMENT_CHANNEL_KEY, GUIDE_KEY] })
